...
async function init() {
  await pool.query(`
    ...
    ALTER TABLE discord_antinuke_settings
      ADD COLUMN IF NOT EXISTS punishment_mode TEXT NOT NULL DEFAULT 'ban';

    ALTER TABLE discord_antinuke_settings
      ADD COLUMN IF NOT EXISTS quarantine_role_id TEXT;

    ALTER TABLE discord_antinuke_settings
      ADD COLUMN IF NOT EXISTS quarantine_channel_id TEXT;
  `);
...
async function getGuildSettings(guildId) {
  ...
  return {
    ...
    promotion_threshold: parseInt(
      process.env.PROMOTION_THRESHOLD || '50',
      10
    ),
    owner_roles:
...
async function getAllGuildSettings() {
  const { rows } = await pool.query(
    'SELECT * FROM discord_guild_settings'
  );
  ...
  try {
    ...
    await client.query(`
      INSERT INTO discord_members
        (user_id, guild_id, total_points)
      VALUES ($1, $2, $3)
      ON CONFLICT (user_id, guild_id)
      DO UPDATE SET total_points =
        discord_members.total_points + $3
    `, [userId, guildId, points]);

    await client.query(`
      INSERT INTO discord_transactions
        (user_id, guild_id, points, reason, type, added_by)
...
async function setPoints(userId, guildId, newTotal, reason, addedBy) {
  ...
  try {
    ...
    `, [
      ...
      guildId,
      Math.abs(diff),
      `تعيين من ${prev} إلى ${newTotal} — ${reason}`,
      addedBy,
    ]);
...
async function deleteTransaction(id, guildId) {
  ...
  try {
    ...
    await client.query(
      `DELETE FROM discord_transactions WHERE id = $1`,
      [id]
    );
...
async function addReactionRole(
  ...
  await pool.query(`
    INSERT INTO discord_reaction_roles
      (guild_id, channel_id, message_id, emoji_key, emoji_label, role_id)
    VALUES ($1, $2, $3, $4, $5, $6)
...
async function upsertChannelBackup(guildId, snapshot) {
  ...
  `, [
    ...
    JSON.stringify(snapshot.overwrites || []),
  ]);
}

async function deleteChannelBackup(channelId) {
  await pool.query(
    `DELETE FROM discord_backup_channels WHERE channel_id = $1`,
    [channelId]
  );
}
...
async function deleteRoleBackup(roleId) {
  await pool.query(
    `DELETE FROM discord_backup_roles WHERE role_id = $1`,
    [roleId]
  );
}
...
async function getAntiNukeSettings(guildId) {
  const { rows } = await pool.query(
    `SELECT *
     FROM discord_antinuke_settings
     WHERE guild_id = $1`,
    [guildId]
  );
...
async function setAntiNukeEnabled(guildId, enabled) {
  await pool.query(`
    INSERT INTO discord_antinuke_settings
      (guild_id, enabled)
    VALUES ($1, $2)
...
async function setAntiNukeThreshold(guildId, count, seconds) {
  await pool.query(`
    INSERT INTO discord_antinuke_settings
      (guild_id, threshold_count, threshold_seconds)
    VALUES ($1, $2, $3)
    ON CONFLICT (guild_id)
    DO UPDATE SET
      threshold_count = $2,
...
// ─────────────────────────────────────────────────────────────
// Anti-Nuke: Incident Logs
// ─────────────────────────────────────────────────────────────

async function logAntiNukeIncident(
  guildId,
  executorId,
  kinds,
  actionCount,
  ...
  await pool.query(`
    INSERT INTO discord_antinuke_logs
      (
        guild_id,
        executor_id,
        kinds,
